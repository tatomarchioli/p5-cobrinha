...
p5.prototype.createSprite = function(x, y, width, height, parent) {
...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  /**
  * The parent object or class for this sprite (optional, used for callback functions).
  * @property parent
  * @type {object}
  */
  ...
  /**
  * Callback function, used to notify the parent class when a collision occours.
  * @function onCollision
  */
  this.onCollision = function(other, type, touching){};
        ...
        if(type === 'overlap') {
...

          {
            ...
            if(displacement.x !== 0 || displacement.y !== 0)
            {
              ...
              /*Hack to work with tillemaps*/
              if(displacement.x !==0) {
                 let haveAfter = false;
                 let haveBefore = false;
                 for (let o of others) {
                     if (o.position.y === other.position.y) {
                         if (other.collider.left() === o.collider.right()) {
                             haveBefore = true;
                         }

                         if (o.collider.left() === other.collider.right()) {
                             haveAfter = true;
                         }
                ...
                if ((haveAfter && displacement.x > 0) ||
                    (haveBefore && displacement.x < 0)) {
                    displacement.x = 0;
                }
              }
                 ...
                 let haveBottom = false;
                    for (let o of others) {
                        if (o.position.x === other.position.x) {
                            if (other.collider.top() === o.collider.bottom()) {
                                haveTop = true;
                            }

                            if (o.collider.top() === other.collider.bottom()) {
                                haveBottom = true;
                            }
                    ...
                    if ((haveTop && displacement.y < 0) ||
                        (haveBottom && displacement.y > 0)) {
                        displacement.y = 0;
                    }
              }
              /*End of the hack*/
              ...
              if(this.onCollision !== undefined && typeof this.onCollision === 'function') {
                  this.onCollision.call(this.parent ? this.parent : this, other, type, this.touching);
              }

              // if(other.onCollision !== undefined && typeof other.onCollision === 'function') {
              //     other.onCollision.call(other.parent ? other.parent : other, this, type, this.touching);
              // }
...
function AABB(pInst, _center, _extents, _offset) {
  ...
  this.minkowskiDifference = function(other)
  {
    ...
    var restl = topLeft.add(fullSize.div(2));
    var resfs = fullSize.div(2);
    return new AABB(pInst, restl, resfs);
  };
  ...
  this.closestPointOnBoundsToPoint = function(point)
  {
    ...
    var minDist = abs(point.y - this.min().y);
    var boundsPoint = createVector(point.x, this.min().y);
    ...
    if (abs(this.min().x - point.x) < minDist)
    {
      minDist = abs(this.min.x - point.x);
      boundsPoint = createVector(this.min().x, point.y);
    }
